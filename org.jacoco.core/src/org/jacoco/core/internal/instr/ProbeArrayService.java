...
public final class ProbeArrayService {
	 ...
	 * in the ExecutionData files.
	 * 
	 * @return the unique type name that identifies this array type
	 */
	public static char getFormatVersion() {
		return getProbeZeroInstance().getFormatVersion();
	}
	 ...
	 * 
	 * @param formatVersion
	 *            version the format of the data to read
	 * @param input
	 *            the input source
	 ...
	 */
	public static IProbeArray<?> read(final char formatVersion,
			final CompactDataInput input) throws IOException {
		final IProbeArray<?> zeroInstance = ProbeMode
				.getZeroInstance(formatVersion);
		if (zeroInstance == null) {
			throw new IOException("Unrecognized format version ("
					+ (int) formatVersion + ") in data stream");
		}
