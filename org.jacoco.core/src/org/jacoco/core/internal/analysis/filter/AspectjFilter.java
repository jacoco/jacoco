 ...
 * <p>
 * Every class containing an advice, will get the synthetic methods
 * {@code aspectOf(..)}, {@code hasAspect(..)}. If the advice is a singleton,
 * the class will also get the {@code ajc$postClinit()} method which will be
 * called from the static initializer ({@code <clinit>()}). This filter ignores
 * these 3 method and the call.
 * <p>
 * Every class containing advised methods, where the advice takes a JoinPoint as
 * parameter, will contain the {@code ajc$preClinit()} method which will be
 * called from the static initializer ({@code <clinit>()}). This filter ignores
 * this method, and its call.
 * <p>
 * If the static initializer ({@code <clinit>()}) only contains the calls to
 * {@code ajc$preClinit()} and/or {@code ajc$postClinit()}, its completely
 * ignored.
 * <p>
 * For every JoinPoint which is advised by an around advice, an
 * {@code $AjcClosure}-class is generated (unless the around advice is inlined).
...
public class AspectjFilter implements IFilter {
	...
	/**
	 * This method finds calls to ajc$preClinit() and ajc$postClinit() and
	 * ignores them.
	 *
	 * @param methodNode
	 *            The {@literal <clinit>()}-Method
	 * @param output
	 */
	private void checkStaticInitializer(MethodNode methodNode,
			IFilterOutput output) {
	...
	/**
	 * Checks if the given method has the {@code org.aspectj.weaver.AjSynthetic}
	 * attribute.
	 *
